...
export default {
  ...
  methods: {
    ...
    zTreeOnText (event, treeId, treeNode) {
      const self = this
      if (treeNode.children) {
        self.zTreeObj.expandNode(treeNode)
      } else {
        self.zTreeObj.checkNode(treeNode, !treeNode.checked, true) // 点击文字选中
        self.$store.commit('moduleForecast/SET_OILfIELD_POINTS', {...treeNode})
      }
    },
    handleCheckedAll(treeNode) {
      let list = this.$refs.zTree.zNodes
      list.forEach((item, index) => {
...
/deep/ .ztree li {
  ...
  /deep/ .ztree li span.button.switch {
    width: 20px;
    height: 20px;
  }
  /deep/ .ztree li span.button.roots_close {
    width: 20px;
    height: 20px;
  }
  /deep/.node_name {
  ...
  /deep/ .checkbox_false_full,
  /deep/ .checkbox_true_full,
  /deep/ .checkbox_true_part,
  /deep/ .checkbox_false_part,
  /deep/ .checkbox_true_part_focus,
  /deep/.checkbox_false_full_focus,
  /deep/.checkbox_true_full_focus,
  /deep/.checkbox_false_part_focus {
